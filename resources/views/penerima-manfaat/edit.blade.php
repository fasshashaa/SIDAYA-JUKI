...
@section('content')
    ...
    <div class="max-w-4xl">
        <form action="{{ route('penerima-manfaat.update', $penerima->id) }}" method="POST" class="space-y-6">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
                ...
                <div>
    <label class="block text-sm font-semibold text-gray-900 mb-2">Pilih Akun Pengguna<span class="text-rose-500">*</span></label>
    <select name="user_id" required class="w-full rounded-xl border-gray-200 bg-gray-50/50 focus:bg-white focus:border-blue-500 focus:ring-blue-500 text-sm p-3 transition-all">
    ...
    @error('user_id') <span class="text-xs text-rose-500 mt-1.5 flex items-center gap-1"><svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/></svg>{{ $message }}</span> @enderror
                    ...
                    <div>
                        ...
                        @error('nama_lengkap') <span class="text-xs text-rose-500 mt-1.5 block">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('nama_ibu_kandung') <span class="text-xs text-rose-500 mt-1.5 block">{{ $message }}</span> @enderror
                    </div>

                    <!-- NIK -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">NIK <span class="text-rose-500">*</span></label>
                        <input type="text" name="nik" maxlength="16" required class="w-full rounded-xl border-gray-200 bg-gray-50/50 focus:bg-white focus:border-blue-500 focus:ring-blue-500 text-sm p-3 font-mono transition-all" value="{{ old('nik', $penerima->nik) }}">
                        @error('nik') <span class="text-xs text-rose-500 mt-1.5 block">{{ $message }}</span> @enderror
                    </div>

                    <!-- No KK -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Nomor Kartu Keluarga <span class="text-rose-500">*</span></label>
                        <input type="text" name="no_kk" maxlength="16" required class="w-full rounded-xl border-gray-200 bg-gray-50/50 focus:bg-white focus:border-blue-500 focus:ring-blue-500 text-sm p-3 font-mono transition-all" value="{{ old('no_kk', $penerima->no_kk ?? '') }}">
                        @error('no_kk') <span class="text-xs text-rose-500 mt-1.5 block">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="no_wa" class="w-full rounded-xl border-gray-200 bg-gray-50/50 focus:bg-white focus:border-blue-500 focus:ring-blue-500 text-sm p-3 pl-10 transition-all" value="{{ old('no_wa', $penerima->no_wa) }}">
                        </div>
                        @error('no_wa') <span class="text-xs text-rose-500 mt-1.5 block">{{ $message }}</span> @enderror
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="kecamatan_select" name="kecamatan" required class="w-full rounded-xl border-gray-200 bg-gray-50/50 text-sm p-3 focus:bg-white focus:border-blue-500 focus:ring-blue-500 transition-all">
                        ...
                        @error('kecamatan') <span class="text-xs text-rose-500 mt-1.5 block">{{ $message }}</span> @enderror
                    </div>

                    <!-- Desa -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Desa / Kelurahan <span class="text-rose-500">*</span></label>
                        <select id="desa_select" name="desa" required class="w-full rounded-xl border-gray-200 bg-gray-50/50 text-sm p-3 focus:bg-white focus:border-blue-500 focus:ring-blue-500 transition-all">
                            <option value="">Pilih Kecamatan dahulu</option>
                        </select>
                        @error('desa') <span class="text-xs text-rose-500 mt-1.5 block">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        @error('alamat_detail') <span class="text-xs text-rose-500 mt-1.5 block">{{ $message }}</span> @enderror
                    </div>
                </div>
            </div>

            {{-- ================= SECTION: STATUS VERIFIKASI ================= --}}
            <div class="bg-gradient-to-br from-blue-50/70 to-indigo-50/40 p-8 rounded-3xl border border-blue-100">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-blue-900">Status Verifikasi <span class="text-rose-500">*</span></label>
                        <p class="text-xs text-blue-600/70">Pastikan data sudah diperiksa sebelum mengubah status</p>
                    </div>
                </div>
